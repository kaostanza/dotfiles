...
(set-face-attribute 'default nil :font "Iosevka" :height 150) ; Setup font and font size
(setq custom-safe-themes t) ; Don't care about the danger of themes
...
(evil-define-key 'normal global-map (kbd "SPC f") 'find-file)
(evil-define-key 'normal global-map (kbd "SPC s") 'save-buffer)
(evil-define-key 'normal global-map (kbd "SPC d") 'dired)
(evil-define-key 'normal global-map (kbd "SPC c") 'compile)
(evil-define-key 'normal global-map (kbd "SPC g") 'keyboard-escape-quit)
(evil-define-key 'normal global-map (kbd "SPC b") 'switch-to-buffer)
(evil-define-key 'normal global-map (kbd "SPC k") 'kill-buffer)
